...
export class BootScene extends Phaser.Scene {
  ...
  preload(): void {
    this.load.json('font-meta', 'fonts/pixel-5x7.json');
    this.load.image('font-image', 'fonts/pixel-5x7.png');
    ...
    this.load.spritesheet('helper', 'sprites/helper.png', { frameWidth: 12, frameHeight: 24 });
    for (const key of ['hall-bg', 'storm-tree-bg', 'water-basement-bg']) this.load.image(key, `scenes/${key}.png`);

    const bar = this.add.rectangle(GAME_WIDTH / 2, GAME_HEIGHT / 2, 0, 6, PALETTE.yellow);
    this.load.on('progress', (p: number) => (bar.width = p * 120));
  }
  ...
  private showTitle(): void {
    ...
    this.add.rectangle(0, 60, GAME_WIDTH, 4, PALETTE.yellow).setOrigin(0);
    this.add.bitmapText(GAME_WIDTH / 2, 28, FONT, 'EINSATZBEREIT').setOrigin(0.5, 0).setScale(2).setTint(PALETTE.white);
    this.add.bitmapText(GAME_WIDTH / 2, 70, FONT, 'Du führst einen THW-Ortsverband.\nDrei Einsätze. Keine Toten. Los.').setOrigin(0.5, 0).setCenterAlign().setTint(PALETTE.grayLight);

    const canContinue = hasSave() && load();
    const options = canContinue ? ['Weiter', 'Neu starten'] : ['Neues Spiel'];
    options.forEach((label, i) => {
      const y = 110 + i * 16;
      const row = this.add.rectangle(GAME_WIDTH / 2, y, 120, 13, PALETTE.thwBlue).setStrokeStyle(1, PALETTE.yellow).setInteractive({ useHandCursor: true });
    ...
    this.add.bitmapText(GAME_WIDTH / 2, GAME_HEIGHT - 22, FONT, 'Maus / Touch: klicken. Fahren: Pfeile oder WASD, Leertaste = Sondersignal').setOrigin(0.5, 0).setTint(PALETTE.gray).setMaxWidth(GAME_WIDTH - 10).setCenterAlign();
  }
}
