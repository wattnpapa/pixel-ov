...
export abstract class SideSceneBase extends Phaser.Scene {
  ...
  protected setupSide(bgKey: string, groundY: number, helperX: number): void {
    ...
    this.bar = new ProgressBar(this, helperX, groundY - 136);
    this.hoverBg = this.add.rectangle(0, 0, 40, 44, PALETTE.black, 0.8).setOrigin(0, 1).setDepth(600).setVisible(false);
  ...
  protected addHotspot(def: HotspotDef, onClick: (def: HotspotDef) => void): Phaser.GameObjects.Rectangle {
    const zone = this.add
      ...
      .setOrigin(0)
      .setDepth(40)
      .setStrokeStyle(4, PALETTE.yellow, 0.25)
      .setInteractive({ useHandCursor: true });
    zone.setData('def', def);
    zone.on('pointerover', () => {
      zone.setStrokeStyle(4, PALETTE.yellow, 0.9);
      this.showHover(def);
    });
    zone.on('pointerout', () => {
      zone.setStrokeStyle(4, PALETTE.yellow, 0.25);
      this.hideHover();
    });
    zone.on('pointerdown', () => {
  ...
  private showHover(def: HotspotDef): void {
    ...
    const w = this.hoverLabel.width + 24;
    let x = def.x;
    if (x + w > GAME_WIDTH - 8) x = GAME_WIDTH - 8 - w;
    const y = Math.max(88, def.y - 8);
    this.hoverBg.setPosition(x, y).setSize(w, 44).setVisible(true);
    this.hoverLabel.setPosition(x + 12, y - 4);
  }
  ...
  protected doAction(label: string, ms: number, onProgress?: (p: number) => void): Promise<void> {
    return new Promise((resolve) => {
      this.bar.setPosition(this.helper.x, this.groundY - 136);
